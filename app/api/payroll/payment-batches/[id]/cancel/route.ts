...
import { cancelPaymentBatch } from "@/lib/payroll-payments-db";
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const data = await cancelPaymentBatch(id, await actorFrom(req));
    return NextResponse.json({ data, error: null });
  } catch (e) {
    return NextResponse.json({ error: e instanceof Error ? e.message : "ยกเลิกชุดจ่ายไม่สำเร็จ" }, { status: 500 });
  }
}

...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  let b: { name?: string; default_w?: number | string | null; default_h?: number | string | null; unit?: string; drive_subpath?: string | null };
  ...
  if (b.drive_subpath !== undefined) patch.drive_subpath = String(b.drive_subpath ?? "").trim() || null;
  ...
  const { data, error } = await admin.from("erp_print_types").update(patch).eq("id", id)
    .select("id, code, name, default_w, default_h, unit, drive_subpath, sort_order").single();

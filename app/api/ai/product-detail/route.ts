...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const system = [
    ...
    ...(hasCustom ? [
      ...
      // ⚠️ เดิมเขียนรวมว่า 'ใช้ข้อมูลจริงให้หมด' → AI เอารายการสีทั้งหมดไปต่อท้ายชื่อสินค้า
      "- ข้อมูลจริงด้านล่าง (น้ำหนัก ประกัน ไซซ์ สี) ให้ใช้ในช่อง introduction/description — ห้ามยัดลงในชื่อสินค้า เว้นแต่คำสั่งเฉพาะช่องชื่อสินค้าสั่งให้ใส่",
      "- ชื่อสินค้าให้ทำตามคำสั่งเฉพาะช่องเป๊ะ ๆ ห้ามเติมอะไรเกินกว่าที่คำสั่งบอก (ห้ามไล่รายชื่อสีทุกสี)",
    ] : [
    ...
    ...(ruleTopics.length ? ["", "หัวข้อที่ต้องมีใน Description เสมอ (ถ้าดูจากรูปไม่ออก ให้ถามใน questions):",
      ...ruleTopics.map((t) => `- ${t}`)] : []),
    // ── คำสั่งรายช่อง = ตัวสุดท้ายจริง ๆ ของ prompt (ตำแหน่งที่โมเดลให้น้ำหนักสูงสุด) ──
    //    ต้องอยู่หลังทั้งคำสั่งรวมและกฎประเภทสินค้า ไม่งั้นสองอันนั้นจะกลบทับ
    ...(hasFieldPrompts ? [
      "",
      ...
      "",
      "ขั้นตอนบังคับก่อนส่งคำตอบ: ไล่คำสั่งเฉพาะช่องข้างบน 'ทีละข้อ' เทียบกับข้อความที่เขียน",
      "ถ้าข้อไหนยังไม่ผ่าน (ลำดับคำผิด · ขาดคำที่สั่งให้มี · ยาวเกินที่กำหนด · มีของที่สั่งห้าม) ให้แก้แล้วตรวจใหม่ก่อนส่ง",
    ] : []),
  ].join("\n");
  ...
  try {
    // ย้ำคำสั่งรายช่องเป็น "ข้อความสุดท้าย" หลังรูป — โมเดลให้น้ำหนักท่อนท้ายของ user message มากที่สุด
  ...
  const missingEn = ([
    ["name_th", "name_en"], ["introduction", "introduction_en"], ["description", "english_description"],
  ] as const).filter(([th, en]) => data[th] && !data[en]);
  if (missingEn.length) {
    try {
      const enOut = await chatJson(
        [
          "คุณคือนักแปลไทย→อังกฤษสำหรับหน้าสินค้าอีคอมเมิร์ซ",
          "แปลให้ตรงความหมายและคงโครงสร้างเดิมเป๊ะ ๆ (ลำดับคำ จำนวนบรรทัด หัวข้อ สัญลักษณ์)",
          "คำที่เป็นภาษาอังกฤษอยู่แล้ว (ชื่อแบรนด์ Collection รหัสสินค้า คำ SEO) ให้คงไว้เหมือนเดิม ห้ามแปลหรือสลับที่",
          "ห้ามเพิ่มข้อมูลใหม่ ห้ามตัดข้อมูลทิ้ง · ตอบเป็น JSON เท่านั้น ตามคีย์ที่ให้มา",
        ].join("\n"),
        [{ type: "text", text: [
          `ตอบ JSON: {${missingEn.map(([, en]) => `"${en}":"ฉบับแปลอังกฤษ"`).join(",")}}`,
          ...missingEn.map(([th, en]) => `${en} ← แปลจากข้อความนี้:\n${data[th]}`),
        ].join("\n\n") }],
        1200, model,
      );
      for (const [, en] of missingEn) {
        const v = String(enOut[en] ?? "").trim();
        if (v) (data as Record<string, unknown>)[en] = v.slice(0, en === "name_en" ? 200 : 4000);
      }
    } catch { /* แปลรอบสองไม่สำเร็จ → ปล่อยว่างไว้ ผู้ใช้กดปุ่มแปลในฟอร์มได้ */ }
  }

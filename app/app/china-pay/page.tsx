...
// ตารางจับคู่ "เหตุการณ์ → กลุ่ม LINE" ค่าเริ่มต้น (ตั้งเองได้ในหน้าตั้งค่า)
const LINE_DEFAULT_ROUTING: Record<string, string> = { rate: "transfers", china_bill: "bills", transfer: "transfers" };
...
function MenuSettings({ onSaved }: { onSaved: (c: Record<string, string[]>) => void }) {
  ...
  // ตารางจับคู่ "เหตุการณ์ → กลุ่ม" (rate/china_bill/transfer → test|bills|transfers|off)
  const [lineRouting, setLineRouting] = useState<Record<string, string>>(LINE_DEFAULT_ROUTING);
  ...
  useEffect(() => {
    apiFetch("/api/master-v2/china-app-settings?limit=20").then(r => r.json()).then(j => {
      ...
      if (lrow) {
        ...
        const rt = (v.routing ?? {}) as Record<string, string>;
        setLineRouting({ ...LINE_DEFAULT_ROUTING, ...rt });
      }
    }).catch(() => {});
  }, []);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        ...
        {/* ตารางจับคู่: เหตุการณ์ → กลุ่ม (ตั้งเองได้) */}
        <div className="mt-4">
          <Label>เหตุการณ์ไหน → ยิงเข้ากลุ่มไหน</Label>
          <div className="space-y-1.5 mt-1">
            {([
              { key: "rate", icon: "📩", label: "ขอเรท" },
              { key: "china_bill", icon: "📦", label: "ส่งบิลจีน / ใบสรุปบิล" },
              { key: "transfer", icon: "💸", label: "ส่งยอดโอน / ใบสรุปการโอน" },
            ] as const).map(ev => (
              <div key={ev.key} className="flex items-center gap-2">
                <span className="flex-1 min-w-0 text-sm text-slate-600 truncate">{ev.icon} {ev.label}</span>
                <span className="text-slate-300 text-xs">→</span>
                <select value={lineRouting[ev.key] ?? "transfers"} onChange={e => setLineRouting(p => ({ ...p, [ev.key]: e.target.value }))}
                  className="flex-shrink-0 w-36 h-9 px-2 text-sm border border-slate-200 rounded-lg bg-white">
                  <option value="test">🧪 กลุ่มทดสอบ</option>
                  <option value="bills">📦 กลุ่มส่งบิล</option>
                  <option value="transfers">💸 กลุ่มส่งยอดโอน</option>
                  <option value="off">🚫 ไม่ส่ง</option>
                </select>
              </div>
            ))}
          </div>
          <div className="text-[11px] text-slate-400 mt-1">เลือกได้ว่าแต่ละเหตุการณ์จะยิงเข้ากลุ่มไหน หรือ “ไม่ส่ง”</div>
        </div>
...
function AllList({ canDelete }: { canDelete: boolean }) {
  ...
  const sendCombined = async () => {
    ...
    try {
      ...
      const res = await apiFetch("/api/china-pay/line-push", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text, purpose: "china_bill" }) });
      const j = await res.json().catch(() => ({}));
...
function BillDetail({ bill, onClose, onPrinted, onChanged, canDelete }: { bill: Record<string, unknown>; onClose: () => void; onPrinted?: (id: string, at: string) => void; onChanged?: () => void; canDelete?: boolean }) {
  ...
  const sendLine = async () => {
    ...
    setSendingLine(true);
    try {
      const res = await apiFetch("/api/china-pay/line-push", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text, purpose: "china_bill", button: { label: "เปิดใบสรุปบิลจีน", url: link } }) });
      const j = await res.json().catch(() => ({}));
...
async function pushTransferLine(t: Record<string, unknown>, toast: { success: (m: string) => void; error: (m: string) => void }): Promise<void> {
  ...
  try {
    const res = await apiFetch("/api/china-pay/line-push", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text, purpose: "transfer", button: { label: "เปิดใบสรุปการโอน", url: link } }) });
    const j = await res.json().catch(() => ({}));
    if (res.ok) { toast.success("ส่งเข้า LINE กลุ่มแล้ว"); return; }
    if (j.needConfig) toast.error("ยังไม่ได้ตั้งค่า LINE Bot — เปิดให้เลือกกลุ่มเอง");
    else toast.error(j.error ?? "ส่ง LINE ไม่ได้ — เปิดให้เลือกกลุ่มเอง");
...
function TransferPage({ preselect = [], onConsumePreselect }: { preselect?: string[]; onConsumePreselect?: () => void }) {
  ...
  const slipKeys = txSlips.map(s => s.key).filter(Boolean);   // เฉพาะที่มีไฟล์จริง (รายการกรอกมือ key ว่าง)
  ...
  return (
    <div className="space-y-4 pb-[150px]">
      ...
      {step === 1 && (
      <div className="rounded-2xl bg-white/95 backdrop-blur border border-slate-200 p-4 shadow-lg shadow-slate-400/30 sticky top-2 z-30">
      ...
      <Card>
        ...
        {/* รายการเงินที่โอนจริง (แนบสลิป AI อ่านยอด หรือกรอกเอง — รวมได้หลายรายการ) */}
        <div className="flex items-center justify-between mb-1">
          <Label>💸 รายการเงินที่โอนจริง ({txSlips.length})</Label>
        </div>
        {/* ปุ่มเพิ่ม: แนบสลิป (AI อ่านยอด) + กรอกยอดเอง */}
        <div className="flex gap-2">
          <button type="button" onClick={() => slipInputRef.current?.click()} disabled={slipUploading || ocrBusy}
            className="flex-1 h-12 rounded-xl border-2 border-dashed border-violet-300 text-violet-600 text-base font-semibold disabled:opacity-50 active:scale-[0.99] transition">
            ＋ เพิ่มสลิป
          </button>
          <button type="button" onClick={addManualSlip} disabled={slipUploading || ocrBusy}
            className="flex-shrink-0 h-12 px-3 rounded-xl border-2 border-dashed border-slate-300 text-slate-500 text-sm font-medium disabled:opacity-50 active:scale-[0.99] transition">
            ＋ กรอกเอง
          </button>
        </div>
        ...
        <div className="mt-2 space-y-2">
          ...
          {txSlips.length === 0 && (
            <div className="text-center text-slate-300 text-sm py-3">— ยังไม่มีรายการ — กดปุ่มด้านบนเพื่อเพิ่ม</div>
          )}
        </div>
        {/* ยอดรวมโอนจริง (ตัวใหญ่) + validation */}
        <div className="mt-3 rounded-xl bg-slate-50 border border-slate-200 px-3 py-2.5 flex items-center justify-between">
          <span className="text-sm font-medium text-slate-600">รวมโอนจริง</span>
          <span className={`text-2xl font-extrabold ${belowMin ? "text-red-600" : "text-emerald-600"}`}>฿{fmt(+slipSum.toFixed(2))}</span>
        </div>
        {belowMin && <div className="mt-1 text-[11px] text-red-500">* ยอดรวมต้องไม่น้อยกว่า ฿{fmt(+minTransfer.toFixed(2))}{useBalance && chinaCoverThb > 0 ? " (หักยอดคงเหลือแล้ว)" : ""}</div>}
...
function TransferReceiptPopup({ t, onClose, autoSendLine, onDelete, onEdit, onLinkSlips }: { t: Record<string, unknown>; onClose: () => void; autoSendLine?: boolean; onDelete?: () => void; onEdit?: () => void; onLinkSlips?: () => void }) {
  ...
  const sendLineImage = async () => {
    ...
    try {
      ...
      const res = await apiFetch("/api/china-pay/line-push", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text, purpose: "transfer", imageUrl, imageUrls: slipUrls, button: { label: "เปิดใบสรุปการโอน", url: link } }) });
      const j = await res.json().catch(() => ({}));
...
function ReportPopup({ bill, onClose, onPrinted }: {
  ...
  const sendLineImage = async () => {
    ...
    try {
      ...
      const res = await apiFetch("/api/china-pay/line-push", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text, purpose: "china_bill", imageUrl, imageUrls: slipUrls, button: { label: "เปิดใบสรุปบิลจีน", url: link } }) });
      const j = await res.json().catch(() => ({}));

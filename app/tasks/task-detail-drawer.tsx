              ...
              {(d.drive_folder_url || d.final_asset_url || d.published_url) && (
                <div className="flex flex-wrap gap-2">
                  ...
                  {d.drive_folder_url && <button disabled={driveBusy}
                    ...
                    className="text-xs px-2.5 py-1 rounded-lg border border-slate-200 text-slate-500 hover:text-violet-700 hover:bg-violet-50 disabled:opacity-50" title={t("อัปไฟล์ที่ยังไม่ขึ้น Drive · ถ้าโฟลเดอร์ถูกลบไปแล้ว จะสร้างใหม่ให้อัตโนมัติ", "Upload files not yet on Drive · recreates the folder automatically if it was deleted")}>{driveBusy ? "..." : `🔄 ${t("ซิงค์/สร้างใหม่", "Sync/Recreate")}`}</button>}

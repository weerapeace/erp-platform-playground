  ...
  return (
    <div className={`relative ${hasPageBg ? "rounded-2xl p-3 sm:p-4" : ""}`} style={{ ...pageStyle(theme.page), fontFamily: fontFam, zoom } as CSSProperties}>
      ...
      <div className={`relative rounded-2xl overflow-hidden shadow-sm text-white ${theme.anim?.heroGradient && theme.hero.mode === "gradient" ? "ov-anim-gradient" : ""}`} style={heroStyle(theme.hero)}>
        ...
        <DashboardPet petUrl={theme.hero.petUrl ?? null} lottieUrl={theme.hero.petLottieUrl ?? null} lottieData={theme.hero.petLottieData ?? null} frames={theme.hero.petFrames ?? null} cfg={theme.pet ?? { notify: false, overdue: true, review: true, dueToday: true, newTasks: true }} data={petData} onAlert={onPetAlert}
          onMove={(pos) => onThemeChange({ ...theme, pet: { ...(theme.pet ?? { notify: false, overdue: true, review: true, dueToday: true, newTasks: true }), posX: pos.x, posY: pos.y } })} />

...
// ============================================================
// คิวรอตรวจ/อนุมัติ — ตารางงานย่อยที่ "ส่งมาแล้ว" (submitted) + popup ดูรูป/อนุมัติ/ตีกลับ
// ของกลาง: StandaloneShell, ERPModal, ImageLightbox, AssigneeStack
// ข้อมูล: /api/creative-tasks/review-queue · อนุมัติผ่าน updateSubtask (สิทธิ์เช็คฝั่ง server)
// ============================================================
...
export default function ReviewQueuePage() {
  ...
  const act = async (status: string, comment?: string) => {
    ...
    try {
      ...
      setActive(null); setLb(-1); await load();
  ...
  return (
    <StandaloneShell title={t("รอตรวจ/อนุมัติ", "Review queue")} icon="🟡" accent="violet">
      ...
      <div className="px-4 sm:px-8 py-6">
        ...
        {items === null ? <div className="py-20 text-center text-slate-400">{t("กำลังโหลด...", "Loading...")}</div>
          : rows.length === 0 ? (

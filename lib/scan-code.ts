...
/**
 * รูปแบบเลขเอกสารจริงในระบบ (ตรวจกับข้อมูลใน DB จริง 2026-07-31)
 * ⚠️ ของจริงไม่ได้เป็นรูปแบบเดียว — ใบเก่า/ใบที่มาจากช่องทางอื่นหน้าตาต่างกัน:
 *    PO 20/90 ใบเป็น P00181 (ระบบเดิม) · PR 54/182 ใบเป็น PR-LINE-18 (สร้างจาก LINE)
 * รูปแบบนี้เป็นแค่ "คำใบ้ว่าน่าจะเป็นอะไร" เพื่อค้นตารางที่ถูกก่อน —
 * ถ้าเดาผิด ตัวค้น (/api/scan/resolve) จะไล่หาตารางอื่นต่อจนเจอ
 */
const DOC_RULES: { kind: ScanKind; re: RegExp }[] = [
  { kind: "po", re: /^PO-\d{4}-\d+$/i },
  { kind: "mo", re: /^MO-\d{4}-\d+$/i },
  { kind: "pr", re: /^PR-\d{4}-\d+$/i },
  { kind: "po", re: /^P\d{4,}$/i },        // ใบสั่งซื้อระบบเดิม เช่น P00181
  { kind: "pr", re: /^PR-[A-Z]+-\d+$/i },  // ใบขอซื้อจาก LINE เช่น PR-LINE-18
];

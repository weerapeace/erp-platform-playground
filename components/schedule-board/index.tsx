...
export function ScheduleBoard<T extends { id: string }>({
  ...
  filters, getFilter, backlogTitle = "ยังไม่ลงวันที่", hint, maxPerDay = 3,
}: {
  items: T[];
  getDate: (i: T) => string | null;
  onSchedule: (i: T, date: string | null) => void;
  renderCard: (i: T) => React.ReactNode;    // การ์ดในกล่อง backlog
          ...
          {cells.map((d, i) => {
            ...
            return (
              <div key={i}
                ...
                onDrop={() => drop(key)}
                className={`min-h-[74px] rounded-lg border p-1 transition-colors ${on ? "border-blue-400 bg-blue-50 ring-1 ring-blue-300" : isToday(d) ? "border-blue-300 bg-blue-50/40" : "border-slate-100 bg-white"}`}>
                <div className={`text-[11px] mb-0.5 ${isToday(d) ? "text-blue-700 font-semibold" : "text-slate-400"}`}>{d.getDate()}</div>
                <div className="space-y-0.5">
                  {dayItems.slice(0, maxPerDay).map((it) => <div key={it.id} {...dragProps(it)}>{renderChip(it)}</div>)}
      ...
      <div
        ...
        {backlog.length === 0
          ...
          : <div className="space-y-1.5 max-h-[62vh] overflow-y-auto pr-0.5">
              {backlog.map((it) => <div key={it.id} {...dragProps(it)}>{renderCard(it)}</div>)}
            </div>}
      </div>
    </div>
  );

  ...
  return (
    <ERPModal open={open} onClose={() => !saving && onClose()} size="xl" storageKey="bom-change-request"
      ...
      <div className="space-y-2">
        ...
        {/* ปุ่มเพิ่มรายการ "ชุดเดียวของหน้านี้" — อยู่ตรงนี้เพราะแถบในตารางจะซ่อนตัวตอนสูตรยังว่าง */}
        <div className="flex flex-wrap items-center gap-2">
          <button onClick={() => setLines((s) => [...s, emptyLine()])}
            className="h-9 px-3 text-[13px] font-medium border border-indigo-200 text-indigo-700 bg-white rounded-lg hover:bg-indigo-50">
            ＋ เพิ่มวัตถุดิบ <span className="text-indigo-400 font-normal">(เลือกจากคลัง)</span>
          </button>
          <button onClick={() => setLines((s) => [...s, { ...emptyLine(), free_text: true }])}
            ...
            className="h-9 px-3 text-[13px] font-medium border border-amber-300 text-amber-800 bg-amber-50 rounded-lg hover:bg-amber-100">
            ✏️ พิมพ์ชื่อเอง <span className="text-amber-600 font-normal">(ไม่รู้รหัส)</span>
          </button>
        </div>

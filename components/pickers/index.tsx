            ...
            {loading ? (
              ...
              list.map(p => (
                <button key={p.id} type="button" onClick={() => select(p)}
                  className={`w-full px-3 py-2 flex items-center gap-2.5 hover:bg-blue-50 transition-colors text-left ${value?.id === p.id ? "bg-blue-50" : ""}`}>
                  <ImageThumbnail url={p.primary_image_url} size={36} alt={p.name} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      {p.sku && <span className="font-mono text-xs bg-slate-100 px-1.5 py-0.5 rounded text-slate-600">{p.sku}</span>}
                      <span className="text-sm text-slate-800 truncate">{p.name}</span>
                    </div>
                    <div className="text-xs text-slate-400 mt-0.5">
                      {p.uom_name && <>หน่วย: {p.uom_name}</>}
                      {p.list_price != null && p.list_price > 0 && <> · ฿{Number(p.list_price).toLocaleString("th-TH")}</>}
                      {p.stock_on_hand != null && <> · คงเหลือ {Number(p.stock_on_hand).toLocaleString("th-TH")}</>}
                    </div>
                  </div>
                </button>
              ))
